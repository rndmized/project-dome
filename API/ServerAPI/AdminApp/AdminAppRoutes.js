...
/** Ban User [Protected Route]
 * 
 * Bans a given user of entering the game. Efectively sets tu user's status to Banned.
 * Returns a success message.
 */
app.post('/banUser', tokenValidator, function (req, res) {
    jwt.verify(req.token, tempSecretKey, (err, authData) => {
        if (err) {
            res.sendStatus(403);
        } else {
            /** Query database to find the user and provide the fields to update such user. */
...
/** Pardon User [Protected Route]
 * 
 * Changes user status to Allowed.
 */
app.post('/pardonUser', tokenValidator, function (req, res) {
    jwt.verify(req.token, tempSecretKey, (err, authData) => {
        if (err) {
            res.sendStatus(403);
        } else {
