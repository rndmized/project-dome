...
listPlayers(): Observable<any> {
  ...
  return this.http.post(this.gameServerURL + 'listPlayers', {token : this.authenticationService.token})
      .map((response: Response) => {
          let res = response.json();
          let onlinePlayers = [];
          res.forEach(element => {
...
kickPlayer( player_ID : string , char_ID : string): Observable<boolean> {
  return this.http.post(this.gameServerURL + 'changeSettings', { player_ID : player_ID, char_ID: char_ID , token: this.authenticationService.token })
      .map((response: Response) => {
          let res = response.json();
          if (res.success) {
            /** DISPLAY MESSAGE ?? */
